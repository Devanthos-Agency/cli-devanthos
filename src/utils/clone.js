...
const validateDirectory = projectName => {
    ...
    // Si el nombre es ".", instalar en el directorio actual
    if (projectName === ".") {
        if (existsSync(path.join(projectPath, "package.json"))) {
            throw new Error(`❌ Ya existe un proyecto en el directorio actual (se encontró package.json).
                
            💡 Opciones:
            • Usa un nombre diferente para crear una subcarpeta
            • Elimina o limpia el directorio actual`);
        }
        return projectPath;
    }
...
const cleanupOnError = (projectPath, projectName) => {
    // No eliminar el directorio actual si se usó "."
...
export async function cloneTemplate(framework, projectName) {
    ...
    try {
        // Crear directorio del proyecto (no aplica si se usa el directorio actual)
        ...
        cleanupOnError(projectPath, projectName);
